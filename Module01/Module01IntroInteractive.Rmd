...
* $\theta_i$ is called the canonical parameter and is a parameter of interest (and will later be a function of covariates)

* $\phi$ is called a nuisance parameter: it is not of interest to us and thus a nuisance (plage)
...
Slightly different ways of writing the exponential family exists, but we will use this version in our course.
...
Hint: [TMA4245]https://wiki.math.ntnu.no/tma4245/tema/begreper/discrete) and nearly the same parameterization for showing the binomial is member of exponential ([watch this YouTube video for more](https://www.youtube.com/watch?v=7mNrsFr7P_A>).
...
Hint: [TMA4245](https://wiki.math.ntnu.no/tma4245/tema/begreper/discrete) and first part of [Sannsynlighetsmaksimering](https://ntnu.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=735b839f71ed49d99fc3af1f013345fb) might help
</details>
...
<span class="question">Prove that $\text{E}(Y_i)=b'(\theta_i)$ (where $b'(\theta_i)= \frac{d b(\theta_i)}{d \theta_i}$) and $\text{Var}(Y_i)=b''(\theta_i)\cdot \frac{\phi}{w}$.</span>

<details><summary>Hints for the strategy</summary>
Take a sideways approach: differentiate with respect to $\theta$ (to get expressions involving $b'(\theta_i)$ & $b''(\theta_i)$), and integrate over $y$.
</details>

<details><summary>Hints for the implementing the strategy</summary>
Integration by parts, and investigate what $\int_{-\infty}^{\infty} \frac{df}{dy}dy$ is
</details>
...
$$
\log( f(y|\theta)) = y \log\lambda - log(y!) - \lambda = \frac{y \log\lambda - \lambda}{1} - log(y!)
$$
Which might look odd, but means we can easily pick out the terms:
$$
\begin{align}
\theta &= \log \lambda \\
\phi &= 1 \\
...
so $b'(\theta) = e^\theta = \lambda$ and $b''(\theta) = e^\theta = \lambda$
...
$$f_y(y; p, r) = \frac{\Gamma(y + r)}{y! \Gamma(r)} (1-p)^r p^y$$
for $y = 0,1,2,\ldots,$, $r>0$ and $p \in (0,1)$, and where $\Gamma()$ denotes the gamma function. (There
...
$$
\begin{align}
\log f(y|r, p) &=  \log \Gamma(y + r) - \log y! - \log \Gamma(r) + r \log(1-p) + y \log p \\
&= \frac{y \log p + r \log(1-p)}{1} + \log \Gamma(y + r) - \log y! - \log \Gamma(r)
\end{align}
$$
So 
...
$$
\begin{align}
\theta &= \log p, so p=e^\theta \\
\phi &= 1 \\
b(\theta) &= -r \log(1-p) = -r \log(1-e^\theta)\\
w &= 1 \\
c(y,\phi,w) &=  \log \Gamma(y + r) - \log y! - \log \Gamma(r)
\end{align}
$$

</details>

<span class="question">b) Use the general formulas for a exponential family to show that $\text{E}(Y)=\mu=r\frac{p}{1-p}$ and $\text{Var}(Y)=\mu \frac{1}{1-p}$.</span>

<details><summary>Solution</summary>

Starting with $E(y) = b'(\theta)$, where $b(\theta)=-r \log(1-\theta) = -r \log(A)$ (with $A=1-e^\theta$)


\begin{align}
E(y) &= \frac{d (-r \log(1-e^\theta))}{d\theta} =-r \frac{d \log(A)}{dA}\frac{d A}{d\theta} \\
&=-r \frac{1}{1-e^\theta} (-e^\theta) \\
&=r \frac{e^\theta}{1-e^\theta} \\
&= r \frac{p}{1-p}
\end{align}

And now to show that $\text{Var}(Y)=\mu \frac{1}{1-p}$, using $\text{Var}(Y_i)=b''(\theta_i)\cdot \frac{\phi}{w}$. Note that  $\phi=w=1$, so 

\begin{align}
\text{Var}(y) &= \frac{d^2 (-r \log(1-e^\theta))}{d\theta^2}  \\
&=r \frac{d \frac{e^\theta}{1-e^\theta}}{d\theta} \\
&=r \frac{(1-e^\theta ) e^\theta - e^\theta (-e^\theta)}{(1-e^\theta)^2} \\
&= r \frac{e^\theta}{1-e^\theta} \frac{1-e^\theta+e^\theta}{1-e^\theta} \\
&= \mu \frac{1}{1-p} \\
\end{align}
