...
The model is the same, but it is expressed differently, and there are several ways of looking at this:
...
$$
L({\boldsymbol \beta}, \sigma^2 ; {\bf x}_i, y_i) = \frac{1}{\sqrt{2\pi \sigma^2}} e^{-\frac{1}{2 \sigma^2}(y_i - {\bf x}_i {\boldsymbol \beta})^2}
$$

log-likelihood for all of the data:

$$
l({\boldsymbol \beta}, \sigma^2 ; X, {\bf y}) = -\frac{n}{2}\log{2\pi \sigma^2} -\frac{1}{2 \sigma^2}({\bf y} - X {\boldsymbol \beta})^T({\bf y} - X {\boldsymbol \beta})
$$
The score vector is
...
\sum_{i=1}^n s_i(\beta)
$$
CHECK THIS: THE SIGN OF \PSI NEEDS FOLLOWING, AND THE X^T SWAPS SIDES

$$
\begin{align}
s_i(\beta) &= \frac{\partial l(\beta)}{\partial \psi} \frac{\partial \psi}{\partial \beta} \\
...
Note that we can also write the score as $s_i(\beta) = \sigma^{-2} (x_i^T \widehat{y}_i - x_i^T y_i) = \sigma^{-2} x_i^T (\widehat{y}_i - y_i)$
