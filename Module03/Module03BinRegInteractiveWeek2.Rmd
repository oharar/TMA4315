...
We see some output. The estimate of the intercept is `r round(coef(fit0),2)`: this is less than 0, so the mean probability that a woman uses contraception is <0.5 (the response is written as `cbind(success, failure)`: there are other ways of writing it, see the help page for `glm`).
...
$$
w = \frac{(\beta_1 - 0)^2}{a_{kk}(\hat{\boldsymbol{\beta}})}
$$

But we can also work with the square root:

$$
z = \frac{\beta_1 - 0}{\sqrt{a_{kk}(\hat{\boldsymbol{\beta}})}} 
$$
...
They are the same! (unless you calculate by hand and round. Then they are just almost the same)
...
<details><summary>Answer</summary>
I don't. With 16 data points, random can look weird in all sorts of ways.
</details>
...
The problem with interpreting these is that it is easy to over-interpret random noise (this is called [Pareidolia](https://en.wikipedia.org/wiki/Pareidolia)). But we know from the test of the residual deviance that this model does not fit the data well enough, so perhaps there is something going on. Or perhaps this extra variation is random noise (Robert Abelson [in his book](https://www.routledge.com/Statistics-As-Principled-Argument/Abelson/p/book/9780805805284?srsltid=AfmBOorgdpIjsLBH9mVBbOE2WUZ5JI3EKTQHbX9lTqNT7otypBln3TOK) summarised this as "chance is lumpy").
 
</details>

<!---
These links are dead. *sniff*
