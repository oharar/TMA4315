...
A total of 481 flour beetles (*Tribolium confusum*) were exposed to 8 different concentration of CS$_2$ (data on log10-dose). 
...
We thus also have to require that the link function is monotone, and we will soon see that we also need to require that it is twice differentiable.
...
We call $\exp(\beta_1)$ the **odds ratio**, and thus $\beta_1$ is the **log odd ratio**. This is because

\begin{align*}
\frac{\frac{P(Y_i=1\mid x_{i1}+1)}{P(Y_i=0)\mid x_{i1}+1)}}{\frac{P(Y_i=1\mid x_{i1})}{P(Y_i=0)\mid x_{i1})}} &= 
\frac{\exp(\beta_0+\beta_1 x_{i1}+\beta_2 x_{i2}+\cdots + \beta_k (x_{ik}+1)}{\exp(\beta_0+\beta_1 x_{i1}+\beta_2 x_{i2}+\cdots + \beta_k x_{ik})} \\
&= \frac{\exp(\beta_0+\beta_1 x_{i1}+\beta_2 x_{i2}+\cdots) \exp(\beta_k (x_{ik}+1))}{\exp(\beta_0+\beta_1 x_{i1}+\beta_2 x_{i2}+\cdots) \exp(\beta_k x_{ik})} \\
&= \exp(\beta_k (x_{ik}+1) - \beta_k x_{ik})\\ 
&= \exp(\beta_k)
\end{align*}
...
## Infant respiratory disease data
...
## Infant respiratory disease Model
...
## Loglikelihood $l(\boldsymbol{\beta})$ I
...
## Loglikelihood $l(\boldsymbol{\beta})$ II
...
## Loglikelihood $l(\boldsymbol{\beta})$ III
...
## Loglikelihood $l(\boldsymbol{\beta})$ plot?
