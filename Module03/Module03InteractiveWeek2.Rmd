...
We see some output. The estimate of the intercept is `r round(coef(fit0),2)`: this is less than 0.5, so the mean probability is <0.5. 
...
$\beta_1$ is the difference between the two levels of `wantsMore`, i.e. it is the difference in the log odds between women who want more children and those who do not. The odds for a woman who wants more children are `r round(exp(fit1$coefficients)*100)`\% of what they are for those who do not. Put the other way around, they are `r round(exp(-fit1$coefficients),1)` times higher for women who do not want more children.
