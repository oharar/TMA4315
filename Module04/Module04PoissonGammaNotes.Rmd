...
* the number of male horseshoe crabs (satellites) residing nearby a female horseshoe crab 
...
First, the study objects were female horseshoe crabs. Each female horseshoe crab had a male horseshoe crab attached to her in her nest. The objective of the study was to investigate factors that affect whether the female horseshoe crab had any other males, called satellites, residing near her. The following covariates were collected for 173 female horseshoe crabs:
...
<span class="question">Discuss what you see. Any potential covariates to influence `Sa`? Which distribution can `Sa` have?</span>

<details><summary>Answer</summary>
Seems that `C`, `W` and `Wt` might be correlated with `Sa`. Also `W` and `Wt` are very correlated. Difficult to see the distribution of `Sa` since plotting a Poisson variable with different means might not be easy to recognize.
</details>
...
## Example: interpreting parameters for the female horseshoe crabs with satellites
...
## Example: Female horseshoe crabs with satellites
...
### Example: Female horseshoe crabs with satellites - the different tests.
...
**c)**
Let $\hat{\eta}(x_{W})$ be the estimated linear predictor when the width of the carapace is $x_{W}$ and the horseshoe crab is "light medium". What is the distribution of $\hat{\eta}(x_{W})$? 
...
**Remark**: the asymptotic distribution of both statistics (deviance and Pearson) are questionable when there are many low counts. Agresti (1996, page 990) suggest analysing grouped data, for example by grouping by width in the horseshoe crab example.
...
Agresti (1996, page 92) explains: "For our horseshoe crab data set, what if width, weight, colour and spine affect the number of satellites for a female horseshoe crab, and we only fitted a model with width as covariate. Then the horseshoe crabs with a certain width are a mixture of horseshoe crabs of various weights, colours and spine condition - that is, a mixture of several Poisson populations, each with its own mean for the response. This heterogeniety may give an overall response distribution where the variance is greater than the standard Poisson variance. "
