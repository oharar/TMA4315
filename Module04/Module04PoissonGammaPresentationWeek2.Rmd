...
- Examples 
— GLM model 
— loglikelihood, score function and Fisher information matrix 
— asympototic results for $\hat{\boldsymbol{\beta}}$ and Wald, score and LRT.
...
$$
\begin{aligned}
l(\hat{\lambda})-l(\tilde{\lambda})) &=\sum_{i=1}^n [y_i \ln(\hat{\lambda}_i)-\hat{\lambda}_i-\ln(y_i!)] - \sum_{i=1}^n [y_i \ln(\tilde{\lambda}_i)-\tilde{\lambda}_i-\ln(y_i!)] \\
&= \sum_{i=1}^n [y_i \left(\ln(\hat{\lambda})_i - \ln(\tilde{\lambda}_i) \right)-\hat{\lambda}_i-\tilde{\lambda}_i] \\
&= \sum_{i=1}^n [y_i \ln(\hat{\lambda}_i/\tilde{\lambda}_i)- (\hat{\lambda}_i-\tilde{\lambda}_i)] \\
\end{aligned}
$$

($\lambda_i=E(y_i)$ for the model of interest)

The Saturated Deviance is 

$$
D = -2 l(\lambda)=-2\sum_{i=1}^n [y_i \ln(y_i)-y_i-\ln(y!)] 
$$

because $\hat{\lambda}_i = y_i$ for this model
...
&=-2(l(\hat{\lambda})-l(\tilde{\lambda}))=
-2\sum_{j=1}^G(l_j(\hat{\lambda}_j)-l_j(\tilde{\lambda}_j))
\end{aligned}
$$
...
$$
\begin{aligned}
LRT &= D_1 - D_2 =-2(l_1(\hat{\lambda}_1)-l(\tilde{\lambda})) -
(-2(l_2(\hat{\lambda}_2)-l(\tilde{\lambda}))) \\
&=-2(l_1(\hat{\lambda}_1)-l_2(\tilde{\lambda}_2))
\end{aligned}
$$
...
Remark: if $\sum_{i=1}^n y_i=\sum_{i=1}^n \hat{\lambda}_i$ then deviance will be equal to 
$$ D=2 \sum_{i=1}^n y_i\ln(\frac{y_i}{\hat{\lambda}_i})$$
...
$$ X_P^2=\sum_{i=1}^n r_i^2=\sum_{i=1}^n  \frac{(y_j-\hat{\lambda}_i)^2}{\hat{\lambda}_i}$$
where $\hat{\lambda}_i=\exp({\bf x}_i^T\hat{\beta})$.
...
$$
d_i=\text{sign}(y_i-\hat{\lambda}_i)\cdot \left\{ 
2[y_i\ln(\frac{y_i}{\hat{\lambda}_i})-(y_i-\hat{\lambda}_i)]\right\}^{1/2}
$$

where the term $\text{sign}(y_i-\hat{\lambda}_i)$ makes negative residuals possible - and we get the same sign as the _Pearson residuals_
...
that $o_i=y_i$ and $e_i==\hat{\lambda}_i=\exp({\bf x}_i^T\hat{\beta})$.
...
```{r}
m.od=glm(Sa~W, family=poisson(link=log), 
             data=crab)
m.qp=glm(Sa~W, family=quasipoisson(link=log), data=crab)
# summary.glm(m.od)
summary.glm(m.qp)$dispersion

(OverDisp.Dev <- m.od$deviance/m.od$df.residual)
Chi2 <- sum((crab$Sa-fitted(m.od))^2/fitted(m.od))
(OverDisp.Chi2 <- Chi2/m.od$df.residual)

```

---

# Rate models with Counts
...
```{r}
# first age + smoke (but no interaction)
...
If we assume that the original data really comes from a lognormal distribution, then it can be shown that 
