...
ali = read.table(data, header = T)
 names(ali)[3:7] <- c("fish", "invertebrate", "reptile", "bird", "other")
ali
```
---
...
fit.main=vglm(y.data~size+factor(lake),
...
fit.lake=vglm(y.data~factor(lake),family=multinomial, data=ali)
summary(fit.lake)
...
fit.size=vglm(y.data~size,family=multinomial, data=ali)
summary(fit.size)
...
(we will only consider cumulative models - and not sequential models. Ranks would be an example of sequential data)

Ordinal data are where the response is categorical and ordered. e.g. when surveys ask for a preference ("How much did you enjoy TNA4315?": "Loved it", "It was OK", "Hated it"). The intuition behind the modelling is that this is friven by an unobserved latent variable, e.e. there is some continuous level of enjoyment of TMA4315, but this is split up into categories. Now to the model....

An unobservable latent variable $U_i$ drives the observed category $Y_i$.

$$ Y_i=r \Leftrightarrow \theta_{r-1}\le U_i \le \theta_r$$
where these $\theta$s are our unobservable thresholds, and the thresholds are monotonically increasing, $-\infty=\theta_0< \theta_1 < \cdots < \theta_{c+1}=\infty$.
...
where we have a new random variable that has cumulative distribution function (cdf) $F$. No intercept is included due to identifiability issues (a shift in intercept would produce the same effect as negative shift in threshold: you could also think of the thresholds as multiple intercepts).
