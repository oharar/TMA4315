...
This is a count, so we could have used _Poisson regression_, but to make things simpler we assume that the counts are such that we instead can assume a _normal distribution_ (in the next module on generalized linear mixed effects models we will use the Poisson distribution).
...
library(lme4);library(ggplot2);library(ggpubr);library(AED)
data(RIKZ)
...
library(lme4); library(ggplot2); library("AED")
data(RIKZ)
fit=lmer(Richness~NAP +(1|Beach),data=RIKZ)
