...
<span class="question">d) R has several functions to extract different parameters and predictions (BLUPs): `coef()`, `fixef()` and `ranef()`. What estimates does each of them produce? (hint: try them, e.g. `coef(fit2)`, and work out how they differ)</span>

<details><summary>Answer</summary>

`coef()` is annoying, as it doesn't give the fixed effects (as it would with `glm()`). We get those with `fixef()`, which returns $\boldsymbol{\hat{\beta}}$:

```{r}
fixef(fit2)
```

`ranef()` gives us the BLUPs, $E(\pmb{\gamma}_i | \mathbf{y}_i)$, for each boy, so this is a matrix:

```{r}
ranef(fit2)
```

Finally, we can work out what `coef()` does:

```{r}
coef(fit2)
```

The intercepts are about 50, which is `fixef(fit2)['(Intercept)']`: `r round(fixef(fit2)['(Intercept)'], 2)`. So each row is $\boldsymbol{\hat{\beta}} + E(\pmb{\gamma}_i | \mathbf{y}_i)$.
