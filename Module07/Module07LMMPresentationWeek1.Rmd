...
- marginal over the random effects
- what is the distribution of $Y_{ij}$ for a new beach?
- Treat the model as $Y_{ij}=\beta_0+\beta_1x_{ij}+\varepsilon^{*}_{ij}$, where $\varepsilon^{*}_{ij}=\gamma_{0i}+\varepsilon_{ij}$ are not iid
...
$p$ fixed effect covariates, $q$ random effect covariates
...
# Marginal and Conditional Formulations

We can write a conditional formulation if we condition on $\gamma_i$, or a marginal if we do not (in which case we marginalise over the distribution of$\gamma_i$
...
$${\bf Y}_i\sim N({\bf X}_i {\boldsymbol \beta}, \sigma^2{\bf I}+{\bf U}_i {\bf Q} {\bf U}_i^T)$$
...
$${\bf Y}\sim N({\bf X} {\boldsymbol \beta},\sigma^2{\bf I}+{\bf U} {\bf G} {\bf U}^T)$$
