---
title: "TMA4315 Generalized linear models. LMMs II"
...
The integration method constructs a _marginal or restricted log-likelihood_ by integrating ${\boldsymbol \beta}$ out of the likelihood
...
# BLUPs for Random intercept models: $\hat{ {\boldsymbol \gamma}}_i$
...
where $e_i$ is the average residual for the $i^{th}$ group:
...
We observe that each subject’s reaction time increases approximately linearly with the number of sleep-deprived
...
The parameter $\tau_{01}$ gives the covariance between the random intercept and random slope.
