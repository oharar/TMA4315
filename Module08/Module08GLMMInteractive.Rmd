...
This is an analysis that was carried out to look at how plants respond to herbivory, and specifically if there is variation in how well they tolerate it. The species was thale cress, *Arabidopsis thaliana*, which is basically a small weed of no commercial value escept to some plant biologists.
...
Because Arabidopsis is highly selfing, seeds of a single individual served as replicates of that genotype (if you don't understand, don't panic). 

The data also include nuisance variables, including the placement of the plant in the greenhouse, and the method used to germinate seeds. If these influence the response, htey should be in teh model, even if we are not interested in them.
