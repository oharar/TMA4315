---
title: "TMA4315 Generalized linear"
subtitle: "Module 8: Generalized linear mixed effects models (GLMM)"
author: "Mette Langaas, Bob O'Hara and Ingeborg Hem, Department of Mathematical Sciences, NTNU"
...
```{r}
mp1 <- glm(total.fruits ~ nutrient*amd, 
             data=Banta, family="poisson")
```

<span class="question">Write down the assumptions of the model</span>
...
<span class="question">What does this suggest? Are there effects of clipping and nutrient? Is there overdispersion, and if so how does it affect the interpretation?</span>
...
The data seem to be over-dispersed. The residual deviance is much larger than the residual degrees of freedom. We can estimate the overdispersion parameter: it is about `r round(deviance(mp1)/mp1$df.residual, 1)`. This makes us much less certain about any inferences:
...
```{r}
mp2 <- glmer(total.fruits ~ nutrient*amd + (1|gen) + (1|popu),
             data=Banta, family="poisson")
summary(mp2)
```
...
<span class="question">Write down the linear predictor for plant $i$ (you can keep the fixed effects in $\bf{x}_i$, but be explicit about the random effects).</span>
...
<span class="question">What does this new analysis tell us? (a) Does it change our interpretation above? (b) how do you interpret the random effects? </span>

<details><summary>Answer</summary>
(a) The fixed effects are very similar to before, so their interptation doesn't change.

(b) The random effects are both intercepts: the variance for the population is much higher tna. for genotype, so there is more variation between populations than genotypes. 
...
<span class="question">Fit a model with a random effect to overdispersion (you may need to create a new factor). How does this new model compare to the others?</span>
...
mp3 <- glmer(total.fruits ~ nutrient*amd + (1|gen) + (1|popu) + (1|OD),
             data=Banta, family="poisson")
summary(mp3)
...
Finally, the "full" model for this data is much more complicated: it was [taken from here](https://glmm.wdfiles.com/local--files/trondheim/Banta_trondheim.pdf). If you look at this, the first problem is to understand the model, which has categorical variables as random effects.
...
fit1 <- glmer(A ~ log(length) + haulsize + (1 | haul), data = fish, 
              family = binomial(link = "logit")) 
# fit2 <- glmer(A ~ log(length) + haulsize + (1 | haul), data = fish, 
#               nAGQ=2, family = binomial(link = "logit")) 
fit0 <- glm(A ~ log(length) + haulsize, data = fish, 
            family = binomial(link = "logit")) 

summary(fit1)
```

<span class="question">c2) Explain what it means that the model is fitted by maximum likelihood. What does Laplace approximation has to do with the model fit?</span>
