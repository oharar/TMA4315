...
<span class="question">**Q**: Why do we want to assume a Poisson distribution?</span>
\normalsize
...
<span class="question">What is the formula for the black line on the left panel?</span>
**A**: see classnotes.
...
<span class="question">How would you proceed to generate a GLMM data set - e.g. based on the beaches example?</span>
...
<span class="question">c2) *New:* Explain what it means that the model is fitted by maximum likelihood. What does Laplace approximation has to do with the model fit?</span>

<span class="question">Skip d)- not in our reading list to test random effects - so too advanced. ** Instead discuss why this is difficult.</span>

<span class="question">d) The log-likelihood value for the corresponding model without any random effects (which corresponds to a model with $\tau_0^2 = 0$) is `r round(logLik(fit0),2)`. Use this to perform a likelihood ratio test with $H_0: \tau_0^2 = 0$. Calculate the corresponding p-value and draw a conclusion.</span>
