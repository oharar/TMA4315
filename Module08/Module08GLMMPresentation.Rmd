...
summary(fitRI)$coefficients
```

```{r,results="hold", echo=TRUE}
summary(fitRI)$varcor
```
...
```{r,results="hold", echo=TRUE}
fitRIod=glmer(Richness~NAP +(1|Beach) +(1|Sample),data=RIKZ,family=poisson(link=log))
summary(fitRIod)$coefficients
```

```{r,results="hold", echo=TRUE}
summary(fitRIod)$varcor
```
